In this chapter, there is the intention of presenting the state of the art. Meaning that,
theoretical and scientific dependencies of the thesis will be covered. The state of the art
has as its primary objective clarify the area that this work intends to cover and give awareness 
to the concepts that help sustain it. This section will also be used as the head of the project
feats, enhancing its progress and quality. 

\subsection{Health Information Systems}
Traditionally, healthcare institutions were characterized by located, paper-based practices.
This means that treatment decisions and overall management was all done by paper.
However, over the time the diseases became more complex, the populations got bigger and medical
technologies emerged, leading to the need of more structured and efficient systems. \cite{healthcare-systems-evolution}

Nevertheless, the evolution of these systems has been marked by several key milestones. In
the 20th century the national health programs rose and hospital networks aimed to expand medical
services. Also, at the end of the 20th century and at the beginning of the 21st century electronic health
records (EHRs), telemedicine and data-driven decision-making came to life, transforming how information
is stored, shared and used. These innovations improved both efficiency and accessibility but came with a
price: challenges regarding data security, interoperability and privacy. \cite{healthcare-systems-evolution}

Today, the panorama changed: the systems seek transition towards more integrated, patient-centered and 
technology-driven models, where technologies such as Artificial intelligence (AI), Internet of Things (IOT) devices
and Blockchain are reshaping the landscape. Brick by brick these technologies move into enabling secure data
exchange, predictive analytics and personalized treatment approaches, contributing to continuous evolution 
that can translate to quality, accessibility and sustainability improvements. \cite{healthcare-systems-evolution}

\subsection{Blockchain}
Blockchain technology, introduced in 2008 through the cryptocurrency
Bitcoin, has been involving and expanded its influence across numerous sectors within
Information and Communication Technology (ICT), with its usage on the rise in recent years. Its adoption
has been in majority due to the phenomenon of cryptocurrencies,
CDBS (Central Bank Digital Currencies), capital investments and
substantial interesting use cases in blockchain startups which gathered interest and development of
this technology. \cite{cryptocurrencies-phenomenom} \cite{CDBC}

Regarding its architecture, it works by record and verify transactions across a network of participants, without needing a central
Authority.
Its structure is composed of a continuous chain of blocks, each containing a set of validated transactions,
a timestamp and a cryptographic hash of the previous block.
By containing the hash of the previous block it ensures immutability and order, making any alteration in previous records
computationally infeasible.

Participants in the network maintain a copy of the ledger, which ensure both transparency and consistency among
them and new transactions are verified through a consensus mechanism - Proof of Work (PoW), Proof of Stake (PoS) or Raft
as an example - which in group append new blocks to the chain.
Once added to a chain, a block becomes an immutable puzzle peace in the grand scheme of the ledger.

In addition, characteristics that can be found in a blockchain are enormous: immutability, meaning
that is not possible to alter, delete or forge data without a consensus. 
Transparency, where all network participants can access and verify transactions and accountability
is achieved without sacrificing privacy through cryptographic means.
Security, due to the fact that cryptographic hashing and digital signatures do protect data from
tampering and unauthorized access.
Traceability, because every transaction can be tracked back to its origin, which leads to making
auditing and trust easier, important characteristic when it comes to use cases such as supply chaining and
Consensus, knowing that all transactions must be collectively validated using defined algorithms,
ensuring this way reliability and agreement among participants. By combining those, chains offer
a robust paradigm for secure, transparent and trustworthy data management.

In sum, blockchain technology operates by storing information in records distributed in a decentralized manner to all computing
...
the network. The transactions are organized into blocks, each cryptographically linked to the next, forming a
chain of blocks known as the Blockchain. Thanks to its advanced features, this technology offers multiple services, such as traceability, integrity, 
and security, while maintaining public and decentralized information storage, thus preserving privacy.
Despite the associated challenges, ongoing research and developments actively address potential hurdles. As blockchain
technology progresses, its potential applications and advantages are just beginning to unfold \cite{btc} \cite{healthcare-and-blockchain}.
...
its characteristics, it's worth exploring the specific features of each type. \cite{blockchain-in-healthcare}
...
records that anyone on the internet can verify and used to add a transaction block. 
Examples of public blockchains include major project networks like Bitcoin, Ethereum, Tron,
ICP and Solana where even its use case can be very different since, as an example,
Bitcoin is meant to only monetary transactions of information while Ethereum, due to its
...
On the other hand, private blockchains are blockchains in which an organization
or entity centralizes writing permissions while reading permissions can be public or arbitrarily restricted.
...
Finally, let's explore the consortium blockchain. This type comprises a set of organizations or entities
and is partially decentralized. Unlike assigning power to a single entity like in permissioned ones,
...
across heterogeneous organizations in contrast to only permissioned blockchains, precisely because these
...
\begin{figure}[H]
	...
	\caption{State of the art: blockchain types table}
...
\begin{figure}[H]
	...
	\caption{State of the art: blockchain types visual representation, Source: \cite{private-vs-public-vs-consortium}}
	\label{fig:sample-image}
\end{figure}
...
demand for data from the industry and research organizations. Simultaneously, unauthorized sharing,
...
practices in the healthcare ecosystem, such as problems with medications, procedures,
competencies and patient falsification. Given this set of challenges, it is necessary to
explore alternative approaches to the existing ones. With key attributes such as decentralization,
distribution and data integrity, without needing third parties, blockchain technology can surge
as a potential ally to fight these problems. In addition, this technology can also ensure and
...
standardized practices get achieved. The use of smart contracts demonstrates how this
...
trials, ensuring regulatory compliance, and establishing a decentralized framework for sharing
...
Regarding the blockchain influence within the healthcare sector, to understand why
...
and its cryptographic schema and the flexibility of supplying data, because the
...
providing innovative solutions to tackle various challenges within the sector. 
The following sections cover the most important characteristics mentioned so far in detail. \cite{blockchain-distributed-ledger-biomedical}
...
of blocks that reinforces the security and integrity of the recorded data.
...
Blockchain technology can be a powerful ally of healthcare security, since it leverages advanced
encryption, distributed consensus and immutability if everything is well-designed. Putting all features together, 
it may represent a strong resource when it comes to cyber threats, malicious actors within the organization and lack 
of integrity in the possessed data.
...
granular control over the access of healthcare data and by ensuring pseudonym in transactions
which by extension gives the blockchain usage benefits like authentication, immutability, provenance,
robustness, availability, security and privacy where all together improve security, transparency and 
privacy. \cite{privacy-healthcare}
...
interconnected and efficient healthcare ecosystem in a controlled and secure environment. 
...
verifiable , traceable across the network and thereby valid.
...
is consistent and cannot be altered by unauthorized entities or malicious actors.
...
transaction delays that can be caused by third parties, multiple entities involved or any other reasons.
...
efficiency. This ensures smoother stakeholder interactions and minimizes delays in critical stages of healthcare processes.

Although the processes can become simpler, there must be a conscious thought from the organization that depending on the design of
...
organisation, imposing a challenge when it comes to access information.
...
ensure interoperability among all hospital systems, since it helps to share data in a secure and effective way. With its decentralized,
...
centralized data storage because it allows more efficient and secure health data exchange between
different healthcare entities, ensuring data integrity and accessibility. 
...
elsewhere. Since these records are dispersed in a vast number of institutions, this becomes a challenge for seamless sharing, due
...
wants to keep the prices at which it gets the products for its activity. Thus, mechanisms must be created in order to tell what
which entity can access from where. Blockchain offers this if well implemented, creating an enhanced data sharing in healthcare by
establishing a secure and transparent mechanism. It enables a decentralized approach to data storage and sharing, ensuring that
...
anonymity, users can still potentially be identified by analyzing transaction publicly available transaction information.
...
be done at the minimum usage of resources is something that is seemed as challenge, but there must be careful thinking on this
matter. \cite{hyperledger-cost}

\paragraph{H. Standardization} \mbox{}\\
...
actually use already well established best practices that faced the same exact problem, precisely because they were
...
must exist clear guidelines about  which data should be stored on-chain or off-chain. Having on hands this kind of information is
...
\paragraph{I. Cultural Resistance} \mbox{}\\
...
health records (PHRs). These records are usually meant for creation, storage and management of the patient data. Blockchain, if
...
for managing clinical data securely.
...
this standardized solution to multiple organizations across the globe, the potential is immense but so it is the complexity
...
in use of course, and still give patients the control to decide who can access its data, which is a requirement to ensure safe and
...
land their data. Also, in cases where the sources of data are not that valid, data can give bias to those that analyze it, which
...
reality it isn't. To go against these problems, not considering the analysis of the source and doubting the author in question, and
it's purpose, there is the possibility of using blockchain which can serve as a powerful ally against falsification, reducing the omission
...
When it comes to the supply chain, there are a lot of mentions to blockchain but in a particularly more focused on
...
the closely related relationship that was mentioned before. Putting in other words, this product leverages blockchain to not only track
...
analysis, data must be also of fine quality in terms of integrity, verifiably and ownability.
